...
import { formatProjectChannelPath } from '../../lib/cascadePath'
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="grow min-w-0">
            ...
            <div className="flex items-center gap-4 text-[12px] text-dim flex-wrap">
              ...
              <span title="项目渠道">{formatProjectChannelPath(p)}</span>
              <span>管理单位：{p.unitShort}</span>
        ...
        <div className="grid grid-cols-3 gap-4">
          <Card title="项目信息" className="col-span-2">
            <DL cols={2} items={[
              ['项目目标', p.goal],
              ['年度目标', p.year_goal || '—'],
              ['项目渠道', formatProjectChannelPath(p)], ['渠道流程', <span key="f" className="flex flex-wrap gap-1.5">{p.channelFlow.map((s, i) => <Tag key={i}>{s}</Tag>)}</span>],
