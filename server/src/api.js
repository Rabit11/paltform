...
function unwrapSessionToken(raw) {
  const text = String(raw || '').trim();
  if (!text) return '';
  if (text.startsWith('{')) {
    try {
      const o = JSON.parse(text);
      return String(o.sessionToken || o.token || '').trim();
    } catch {
      return text;
    }
  }
...
function sessionToken(req) {
  const direct = unwrapSessionToken(req.header('x-session'));
  if (direct) return direct;
  const authorization = String(req.header('authorization') || '').trim();
  return unwrapSessionToken(authorization.toLowerCase().startsWith('bearer ') ? authorization.slice(7) : '');
}
...
r.get('/admin/users', (req, res) => {
  const admin = assertAdminUser(req, res);
  if (!admin) return;
  const users = db.prepare('SELECT * FROM users ORDER BY role, id').all().map((u) => ({
    ...publicUser(u),
    ...formAccessMeta(u),
  }));
