...
const CHANNELS = CHANNEL_SPEC.map((c, i) => ({
  id: i + 1,
  key: c.key,
  name: c.name,
  level: c.level,
  source_channel: c.sourceChannel,
  org_office: c.orgOffice,
  org: c.orgOffice,
  dept: c.level === '公司级' ? c.orgOffice : '科研项目处',
  flow: c.flow,
  declare: c.declare,
  filing: c.filing,
  assess: c.assess || [],
  chain: c.chain || DECLARE_CHAIN,
  mode: c.mode || '审批',
}));
const chByKey = Object.fromEntries(CHANNELS.map((c) => [c.key, c]));
if (!CHANNELS.length) throw new Error('渠道口径未生成任何渠道叶子');
...
const DEF = [
  ...
  ['民机颤振主动抑制技术研究', 'NSFC', '上飞院', '实施中', 5200, 2024, 2027, 'yellow', null, [['南京航空航天大学', '气动伺服弹性建模']]],
  ...
  ['民机健康管理(PHM)大数据平台', 'ZDYF', '客服公司', '实施中', 7800, 2023, 2026, 'red', null, [['上飞院', '机载数据接口标准']]],
  ['民用飞机试飞数据中心能力建设', 'ZDYF', '试飞中心', '实施中', 12800, 2024, 2027, 'blue', null, []],
  ...
  ['全机静力试验智能测控系统', 'YYGD', '上飞公司', '实施中', 5400, 2024, 2027, 'blue', null, [['基础能力中心', '高精度加载设备']]],
  ['飞机总装脉动生产线仿真优化', 'YYGD', '上飞公司', '实施中', 4100, 2024, 2026, 'yellow', null, []],
